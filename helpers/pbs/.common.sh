...
read_job_state() {
    ...
    status_line=$(timeout "${SCHEDULER_TIMEOUT:-5}" qstat -f "$JOB_ID" 2>/dev/null); exit_code=$?
...
wait_for_job() {
    ...
    NODE=$(timeout "${SCHEDULER_TIMEOUT:-5}" qstat -f "$job_id" 2>/dev/null | grep "exec_host" | cut -d'=' -f2 | cut -d'/' -f1 | tr -d ' ')
    if [ -z "$NODE" ]; then
